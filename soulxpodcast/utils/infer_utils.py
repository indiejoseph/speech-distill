...
def initiate_model(
    seed, model_path, llm_engine, fp16_flow, add_task_token=True, add_speaker_id=True
):
    ...
    if llm_engine == "vllm":
        ...
        if not importlib.util.find_spec("vllm"):
            ...
            tqdm.write(
                f"[{timestamp}] - [WARNING]: No install VLLM, switch to hf engine."
            )

    config = Config(
        model=model_path, enforce_eager=True, llm_engine=llm_engine, hf_config=hf_config
    )
    model = SoulXPodcast(config)

    dataset = PodcastInferHandler(
        model.llm.tokenizer, None, config, add_task_token, add_speaker_id
    )

    return model, dataset


def process_single_tts_input(dataset, texts, prompt_wav_list, prompt_text_list):
    dataitem = {
        "key": "001",
    ...
    data = dataset[0]
    prompt_mels_for_llm, prompt_mels_lens_for_llm = s3tokenizer.padding(
        data["log_mel"]
    )  # [B, num_mels=128, T]
...
def process_single_input(
    ...
    if use_dialect_prompt:
        dataitem.update({"dialect_prompt_text": dialect_prompt_text_list})
    dataset.update_datasource([dataitem])
...
def check_models(model_path, inputs):
    if inputs["use_dialect_prompt"]:
        assert (
            "dialect" in model_path
        ), "Dialect prompt is used, you should use a dialect model."
