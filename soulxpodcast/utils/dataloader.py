...
TEXT_START, TEXT_END, AUDIO_START = (
    "<|text_start|>",
    "<|text_end|>",
...
class PodcastDataset(Dataset):
    ...
    def __init__(
        ...
        tqdm.write(
            f"[{timestamp}] - [INFO] - Loaded {total_lines} lines, found {missing} missing lines, total valid lines == {len(self.datas)}."
        )
        ...
        self.spk_model = onnxruntime.InferenceSession(
            f"{self.model_config.model}/campplus.onnx",
            sess_options=option,
        ...
        try:
            ...
            if self.add_task_token:
                dialect_prefix_list.append(
                    self.text_tokenizer.encode(f"{TASK_PODCAST}")
                )
            for spk_idx, (prompt_text, prompt_wav) in enumerate(
                zip(data["prompt_text"], data["prompt_wav"])
            ):
                ...
                # 4. feature for llm
                speaker_token = SPK_DICT[spk_idx] if self.add_speaker_id else ""
                prompt_text = normalize_text(
                    prompt_text
                )  # remove some space and strange character
                prompt_text = (
                    f"{speaker_token}{TEXT_START}{prompt_text}{TEXT_END}{AUDIO_START}"
                )
                if spk_idx == 0:
                    prompt_text = (
                        f"{TASK_PODCAST}{prompt_text}"
                        if self.add_task_token
                        else prompt_text
                    )
                ...
                if use_dialect_prompt:
                    dialect_prompt_text = normalize_text(
                        data["dialect_prompt_text"][spk_idx]
                    )
                    dialect_prompt_text = f"{speaker_token}{TEXT_START}{dialect_prompt_text}{TEXT_END}{AUDIO_START}"
                    dialect_prompt_text_ids = self.text_tokenizer.encode(
                        dialect_prompt_text
                    )
            ...
            for text, spk in zip(data["text"], data["spk"]):
                ...
                speaker_token = SPK_DICT[spk] if self.add_speaker_id else ""
                text = f"{speaker_token}{TEXT_START}{text}{TEXT_END}{AUDIO_START}"
            ...
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S,%f")[:-3]
            tqdm.write(
                f"[{timestamp}] - [WARNING] - Error processing data item {data.get('key', idx)}: {e}"
            )
            return None
        return item
