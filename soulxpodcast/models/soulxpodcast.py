...
from soulxpodcast.engine.llm_engine import HFLLMEngine, VLLMEngine
...
class SoulXPodcast(torch.nn.Module):
    def __init__(self, config: Config = None):
        ...
        self.audio_tokenizer = (
            s3tokenizer.load_model("speech_tokenizer_v2_25hz").cuda().eval()
        )
        ...
        self.flow.load_state_dict(
            torch.load(
                f"{self.config.model}/flow.pt", map_location="cpu", weights_only=True
            ),
            strict=True,
        )
        self.flow.cuda().eval()
        ...
        hift_state_dict = {
            ...
            for k, v in torch.load(
                f"{self.config.model}/hift.pt", map_location="cpu", weights_only=True
            ).items()
        }
    ...
    def forward_tts2(
        self,
        prompt_mels_for_llm: torch.Tensor,
        prompt_mels_lens_for_llm: torch.Tensor,
        inputs: list[int],
        prompt_mels_for_flow_ori: torch.Tensor,
        spk_emb_for_flow: torch.Tensor,
        sampling_params: SamplingParams | list[SamplingParams],
        **kwargs,  # for compatibility
    ):
        # Ensure batch dimension for quantization
        if prompt_mels_for_llm.ndim == 2:
            prompt_mels_for_llm = prompt_mels_for_llm[None]
        if prompt_mels_lens_for_llm.ndim == 0:
            prompt_mels_lens_for_llm = prompt_mels_lens_for_llm[None]

        # Audio tokenization
        prompt_speech_tokens_ori, prompt_speech_tokens_lens_ori = (
            self.audio_tokenizer.quantize(
                prompt_mels_for_llm.cuda(), prompt_mels_lens_for_llm.cuda()
            )
        )

        # Align speech token with speech feat
        prompt_speech_token_len = prompt_speech_tokens_lens_ori[0].item()
        prompt_speech_token = prompt_speech_tokens_ori[0, :prompt_speech_token_len]

        # prompt_mels_for_flow_ori is expected to be [B, T, D] or [T, D]
        if prompt_mels_for_flow_ori.ndim == 3:
            prompt_mel = prompt_mels_for_flow_ori[0]
        else:
            prompt_mel = prompt_mels_for_flow_ori
        ...
        if prompt_speech_token_len * 2 > prompt_mel_len:
            ...
            prompt_mel = (
                prompt_mel.detach().clone()[: prompt_speech_token_len * 2].cuda()
            )
            prompt_mel_len = torch.tensor([prompt_speech_token_len * 2]).cuda()

        # Prepare LLM inputs
        print("LLM input:", self.llm.tokenizer.decode(inputs))
        ...
        generated_speech_tokens = [
            ...
            for token in llm_outputs["token_ids"][:-1]
        ]  # ignore last eos

        flow_input = torch.tensor(
            [prompt_speech_token.tolist() + generated_speech_tokens]
        )
        ...
        with torch.amp.autocast(
            ...
            generated_mels, generated_mels_lens = self.flow(
                flow_input.cuda(),
                flow_inputs_len.cuda(),
                prompt_mel[None],
                prompt_mel_len[None],
                spk_emb_for_flow.cuda(),
                streaming=False,
                finalize=True,
            )
        ...
        # HiFi-GAN generation
        mel = generated_mels[
            :, :, prompt_mel_len.item() : generated_mels_lens[0].item()
        ]
    ...
    def forward_tts(
        ...
        prompt_mels_for_llm: torch.Tensor,
        prompt_mels_lens_for_llm: torch.Tensor,
        prompt_text_tokens_for_llm: list[int],
        text_tokens_for_llm: list[int],
        prompt_mels_for_flow_ori: torch.Tensor,
        spk_emb_for_flow: torch.Tensor,
        sampling_params: SamplingParams | list[SamplingParams],
        **kwargs,  # for compatibility
        ...
        speech_tokens_i = [
            token + self.config.hf_config.speech_token_offset
            for token in prompt_speech_token.tolist()
        ]
        speech_tokens_i += [self.config.hf_config.eos_token_id]

        # Flatten if they are lists of lists
        prompt_text_tokens = (
            list(chain.from_iterable(prompt_text_tokens_for_llm))
            if isinstance(prompt_text_tokens_for_llm[0], list)
            else prompt_text_tokens_for_llm
        )
        text_tokens = (
            list(chain.from_iterable(text_tokens_for_llm))
            if isinstance(text_tokens_for_llm[0], list)
            else text_tokens_for_llm
        )

        inputs = prompt_text_tokens + speech_tokens_i + text_tokens
    ...
    def forward_longform(
        ...
        **kwargs,  # for compatibility
        ...
        for prompt_index in range(prompt_size):
            ...
            prompt_speech_token = prompt_speech_tokens_ori[
                prompt_index, :prompt_speech_token_len
            ]
        ...
        for i in range(turn_size):
            ...
            if (
                valid_turn_size > self.config.max_turn_size
                or len(inputs) > self.config.turn_tokens_threshold
            ):
                assert (
                    self.config.max_turn_size
                    >= self.config.prompt_context + self.config.history_context
                ), "Invalid Long history size setting, "
                prompt_text_bound = max(
                    self.config.prompt_context,
                    len(history_inputs)
                    - self.config.history_text_context
                ...
                inputs = list(
                    chain.from_iterable(
                        ...
                        + history_inputs[
                            prompt_text_bound : -self.config.history_context
                        ]
                        + prompt_inputs[-self.config.history_context :]
                    )
                )
                valid_turn_size = (
                    self.config.prompt_context + len(history_inputs) - prompt_text_bound
                )
                past_key_values = DynamicCache(config=cache_config)
            valid_turn_size += 1
            ...
            llm_outputs = self.llm.generate(
                inputs, sampling_params, past_key_values=past_key_values
            )

            inputs.extend(llm_outputs["token_ids"])
            prompt_inputs.append(text_tokens_for_llm[i] + llm_outputs["token_ids"])
            history_inputs.append(
                text_tokens_for_llm[i][:-1]
            )  # remove the <|audio_start|>
            ...
            with torch.amp.autocast(
                "cuda",
                dtype=(
                    torch.float16 if self.config.hf_config.fp16_flow else torch.float32
                ),
            ):
                generated_mels, generated_mels_lens = self.flow(
                    flow_input.cuda(),
            ...
            # HiFi-GAN generation
            mel = generated_mels[
                :, :, prompt_mels_lens[0].item() : generated_mels_lens[0].item()
            ]
            wav, _ = self.hift(speech_feat=mel)
